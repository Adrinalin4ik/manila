...
fn parse_server_body(
    ...
    let packet = match opcode {
        ...
        // The Warden body is opaque here — it is RC4 ciphertext this layer does not own — so the
        // arm takes the WHOLE remaining slice and marks it read. `r` is still the untouched body at
        // this point (nothing above advanced it), which is what the pre-refactor `body.to_vec()`
        // named before upstream moved the dispatch onto a cursor. Emptying `r` is not cosmetic:
        // `parse_server_with_tail` reports whatever is left as an unread tail, and an opaque body
        // left unconsumed would report the entire packet as trailing garbage.
